...
@pytest.fixture
def article_data(user_factory, article_factory):
    """
...
@pytest.mark.parametrize(
    ...
    [
        (ArticleStatus.PUBLISH, status.HTTP_201_CREATED, "Shikoyat yuborildi."),
        (ArticleStatus.TRASH, status.HTTP_404_NOT_FOUND, "No Article matches the given query."),
    ]
)
def test_report_article(api_client, tokens, report_data, article_status, expected_status, expected_detail):
    ...
    if article_status == ArticleStatus.PUBLISH:
        response = client.post(f'/articles/{article.id}/report/')
